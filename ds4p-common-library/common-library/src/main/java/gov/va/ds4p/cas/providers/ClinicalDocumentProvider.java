...
public class ClinicalDocumentProvider {
    ...
    private String createClinicalDocumentStringFromObject(POCDMT000040ClinicalDocument doc) throws JAXBException {
      ...
      String res = "";
        ...
        JAXBElement<POCDMT000040ClinicalDocument> element = factory.createClinicalDocument(doc);
        try {
            JAXBContext context = JAXBContext.newInstance(POCDMT000040ClinicalDocument.class);
...
//      marshaller.marshal(factory.createClinicalDocument(doc), sw);
//
//      res = sw.toString();
      return res;
    }
    
    public String createConsentDirective(OrganizationPolicy orgPolicy, String patientName, String patientId, String patientIdType, String patientBirthDate, String patientGender, 
                                         String authorization, String intendedPOU, List<String> allowedPOU, String primaryRecipient, List<String> allowedRecipients, 
                                         List<String> maskingActions, List<String> redactActions, String pdfstring, String xacml) {
        String res = "";
        try {
            clinicalDocument = new POCDMT000040ClinicalDocument();
            

            clinicalDocument.setClassCode(ActClinicalDocument.DOCCLIN);
            clinicalDocument.getMoodCode().add("EVN");
            ...
            clinicalDocument.setTypeId(cId);
            
            //set templates
            List<II> ids = getDocumentTemplates();
            Iterator iter = ids.iterator();
            while (iter.hasNext()) {
                II i = (II)iter.next();
                clinicalDocument.getTemplateId().add(i);
            }
            ...
            clinicalDocument.setId(idV);
            
            // set displayName
            clinicalDocument.setCode(getDocumentCode());
            
            // set type, title, etc
            clinicalDocument.setTitle(getDocumentTitle());
            
            //effective Date
            clinicalDocument.setEffectiveTime(getDocumentEffectiveDate());
            
            clinicalDocument.setConfidentialityCode(getDocumentConfidentialityCode(orgPolicy, maskingActions, redactActions));
            ...
            clinicalDocument.getRecordTarget().add(recordTarget.get(0));
            ...
            clinicalDocument.getInformationRecipient().add(recipient);
            
            //legal authenticator
            POCDMT000040LegalAuthenticator authenticator = getDocumentLegalAuthenticator(orgPolicy, patientName);
            clinicalDocument.setLegalAuthenticator(authenticator);
            
            //documentOf
            POCDMT000040DocumentationOf docOf = getDocumentDocumentOf();
            clinicalDocument.getDocumentationOf().add(docOf);
            

            
            //build remaining structure in getDocumentComponent2            
            ...
            clinicalDocument.setComponent(comp);
            try {
                res = createClinicalDocumentStringFromObject(clinicalDocument);                
            }
            catch (Exception tEx) {
                res = tEx.getMessage();
            }
    ...
    private ST getDocumentTitle() {
        ...
        //st.getNullFlavor().add("Privacy Consent Authorization");
    ...
    private List<POCDMT000040RecordTarget> getDocumentRecordTargets(String patientId, String patientName, String patientGender, String patientBirthDate, OrganizationPolicy orgPolicy) {
        List<POCDMT000040RecordTarget> targets = new ArrayList();
    ...
    private POCDMT000040Component2 getDocumentComponent2(CE conf, String intendedPurposeOfUse, String authorization, OrganizationPolicy orgPolicy, List<String> maskingActions, List<String> redactActions, String pdf, String xacml) {
            ...
            //title
            ST sectTitle = new ST();
            //sectTitle.getNullFlavor().add("Consent Directive Detail");
    ...
    private POCDMT000040InformationRecipient getDocumentInformationRecipient(String primaryRecipient) {
        ...
        // fix this as attribute being passed in
    ...
    private POCDMT000040LegalAuthenticator getDocumentLegalAuthenticator(OrganizationPolicy orgPolicy, String patientName) {
        //lets assume the patient has signed digitally or hardcopy
    ...
    private POCDMT000040DocumentationOf getDocumentDocumentOf() {
        //make default of 5 years in future for testing
